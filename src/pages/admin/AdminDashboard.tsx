...
export const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      setError(null);
      try {
        const [usersRes, teamsRes, tournamentsRes, upcomingRes] = await Promise.all([
          apiFetch<any>("/players?page=1&limit=1"),
          apiFetch<any>("/teams?page=1&limit=1"),
          apiFetch<any>("/tournaments?page=1&limit=1"),
          apiFetch<any>("/tournaments?status=upcoming&limit=3")
        ]);
        setStats([
          { label: "Total Users", value: usersRes.total || usersRes.data?.length || 0, change: "", icon: UsersIcon, color: "text-blue-500" },
          { label: "Active Teams", value: teamsRes.total || teamsRes.data?.length || 0, change: "", icon: UsersIcon, color: "text-green-500" },
          { label: "Live Tournaments", value: tournamentsRes.total || tournamentsRes.data?.length || 0, change: "", icon: TrophyIcon, color: "text-[#f34024]" },
          { label: "Total Prize Pool", value: "$-", change: "", icon: DollarSignIcon, color: "text-yellow-500" }
        ]);
        setUpcomingTournaments(upcomingRes.data || []);
        // Optionally fetch recent activity if API supports
      } catch (err: any) {
